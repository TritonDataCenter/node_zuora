...
test('xml: escapes ZOQL queries', function (t) {
  ...
  t.equal(qs.toLowerCase().indexOf('<![cdata'), -1, 'we choose not to use CDATA to escape queries');
  t.false(qs.match(/\[cdata/),'we choose not to use CDATA to escape queries');
  ...
  t.equal(qs.indexOf('<'), -1, 'the < character cannot be in a query');
