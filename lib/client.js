...
var requestOptions = {timeout: 2 * 60 * 1000};
...
var client = function(config, callback) {
  ...
  soap.createClient(config.wsdl, opts, function(err, client) {
    if (err) {
      return callback(common.makeError(err));
    }
    client.ZuoraService.Soap.login({
      ...
      if (config.verboseLog) {
        client.verbose = true;
        log.info("\n" + (new Date()));
        log.info("Sent:     " + client.lastRequest);
        log.info("Recieved: " + resp.body + "\n");
      }
      if (err) {
        return callback(common.makeError(err));
      }
      ...
      client._requestOptions = requestOptions;
      client._session = mysession; // save for REST call
      client.joyentsession = mysession; // save for REST call (deprecated name for REST queries that haven't updated yet)
      client.addSoapHeader(header, '', 'zns' );
      // TODO: set session timer to renew session?
...
/*
var response = function(err, client, resp, body, callback) {
  var result;
  ...
  if (config.verboseLog) {
    console.log("\nNow: " + (new Date()));
    console.log("response: lastResponseHeaders", JSON.stringify(client.lastResponseHeaders, undefined, 2));
...
// commented out pretty much everything because we just want connecting to work first

/******************************************************************************
var create = module.exports.create = function(existing_client, zObject, callback) {
  var xmlObject = xml.makeCUD('zns:create', zObject);
  // console.log("create: "+xmlObject);
  client(existing_client, function(err, client) {
    if (err) {
      console.log("Byte me .... not able to create here |", err, "|");
      callback(err, client);
    } else {
      client.ZuoraService.Soap.create(xmlObject, function(err, resp, body) {
        response(err, client, resp, body, callback);
      }, reqOptions);
    }
  });
};

var forecast = module.exports.forecast = function(existing_client, zObject, callback) {
  var xmlObject = xml.makeCUD('zns:forecast', zObject);
  // console.log("create: "+xmlObject);
  client(existing_client, function(err, client) {
    if (err) {
...
var query = module.exports.query = function(existing_client, zObject, callback) {
...
var query_all = module.exports.query_all = function(existing_client, zObject, callback) {
  var fullResults;
  ...
  var getMore = function(results, client, cb) {
    ...
    client.ZuoraService.Soap.queryMore(zMore, function(err, resp, body) {
      response(err, client, resp, body, function(err, client, results) {
        if (err) {
  ...
  client(existing_client, function(err, client) {
    ...
    if (err) {
      ...
      client.ZuoraService.Soap.query(zObject, function(err, resp, body) {
        response(err, client, resp, body, function(err, client, results, body) {
          if (err) {
...
var update = module.exports.update = function(existing_client, zObject, callback) {
  var xmlObject = xml.makeCUD('zns:update', zObject);
  client(existing_client, function(err, client) {
    if (err) {
...
var delete = module.exports.delete = function(existing_client, zObject, callback) {
  client(existing_client, function(err, client) {
    if (err) {
...
var execute = module.exports.execute = function(existing_client, zObject, callback) {
  client(existing_client, function(err, client) {
    if (err) {
...
//
var subscribe = module.exports.subscribe = function(existing_client, zObject, callback) {
  var xmlObject = xml.makeCUD('zns:subscribe', zObject);
  client(existing_client, function(err, client) {
    if (err) {
      console.log("Byte me .... not able to query here |", err, "|");
      callback(err, client);
...
var amend = module.exports.amend = function(existing_client, zObject, callback) {
  var xmlObject = xml.makeCUD('zns:amend', zObject);
  client(existing_client, function(err, client) {
    if (err) {
...
var sendfile = module.exports.sendfile = function(existing_client, filename, callback) {
  //TODO need to check to see if content > 4MB if so zip it up
  client(existing_client, function(err, client) {
    var content = fs.readFileSync(filename).toString("base64");
    // TODO:dm this zObject with multiple namespaces may need xml.makeCUD('zns:Import', zObject);
    var zObject = {
      'zns:Import': {
    ...
    create(client, zObject, callback);
  });
};
...
var create_active_account = module.exports.create_active_account = function(account_number, existing_client, values, callback) {
  ...
  create(existing_client, xml.makeZobject('zns:Account', 'ons', zAccount), function(err, client, results) {
    if (test_err_results(err, 'create zAccount', zAccount, results)) {
      ...
      create(client, xml.makeZobject('zns:PaymentMethod', 'ons', zPayment), function(err, client, result) {
        ...
        if (test_err_results(err, 'create zPayment', zPayment, result)) {
          ...
          create(client, xml.makeZobject('zns:Contact', 'ons', zContact), function(err, client, result) {
            if (test_err_results(err, 'create zContact', zContact, result)) {
              ...
              create(client, xml.makeZobject('zns:Contact', 'ons', zSoldContact), function(err, client, result) {
                if (test_err_results(err, 'create zContact', zContact, result)) {
                  var soldContactID = result[0].Id;
                  var zUpdate = make_update_account(accountID, contactID, soldContactID, paymentMethodId, 'Active');
                  update(client, xml.makeZobject('zns:Account', 'ons', zUpdate), function(err, client, result) {
...
var endSubscription = module.exports.endSubscription = function(subscriptionNumber, endTime, client, callback) {
  ...
  get_first_active_subscription(subscriptionNumber, client, function(err, client, sub_id, account_id, datasetname, numFound) {
    ...
    if (numFound == 0) {
      ...
      create(client, xml.makeZobject('zns:Amendment', 'ons', values), function(err, client, result, body) {
...
var get_active_subscription = module.exports.get_active_subscription = function(account_id, machine, existing_client, callback) {
  ...
  query(existing_client, zQuery, function(err, client, result) {
...
var update_account_crm = module.exports.update_account_crm = function(uuid, crmid, existing_client, callback) {
  get_account_id(uuid, existing_client, function(err, client, account_id) {
    if (err) {
      ...
      update(client, xml.makeZobject('ons:Account', 'ons', values), function(err, client, result) {
...
var update_account_sync = module.exports.update_account_sync = function(uuid, value, existing_client, callback) {
  get_account_id(uuid, existing_client, function(err, client, account_id) {
    ...
    if (err) {
      ...
      update(client, xml.makeZobject('zns:Account', 'ons', values), function(err, client, result) {
        if (test_err_results(err, "Update Account", values, result)) {
...
var verifyBillingTags = module.exports.verifyBillingTags = function(usages, theDate, client, callback) {
  ...
  query_all(client, zObject, function(err, client, results) {
...
var export_query = function(maybe_client, query, callback) {
  var client;
  var values = {
    format: 'csv',
    name: 'my export 1',
  ...
  var zObject = xml.makeZobject('zns:Export', 'ons', values);
  try {
    create(maybe_client, zObject, handleResponse);
  } catch (e) {
    console.log(e);
  ...
  function handleResponse(err, valid_client, resp) {
    if (err) {
      verboseLog('create error making export_query')
      return callback(err);
    }
