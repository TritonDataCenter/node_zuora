...
/**
 * Escape user and custom fields into XML
 *
 * 1. Wraps known user-entered fields and custom fields in CDATA
 * 2. Writes Date objects to ISO format
 */
var escapeFields = function(field, value) {
  if (value instanceof Date) {
    value = value.toISOString();
  }
  if (_.includes(userFields, field.toLowerCase()) || _.endsWith(field.toLowerCase(), '__c')) {
    value = cdata(value);
  }
  return value;
};
